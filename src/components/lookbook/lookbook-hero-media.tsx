// src/components/lookbook/lookbook-hero-media.tsx
"use client";
...
type LookbookHeroMediaProps = {
  entry: LookbookEntry;
};

export function LookbookHeroMedia({ entry }: LookbookHeroMediaProps) {
  const videoRef = React.useRef<HTMLVideoElement | null>(null);

  const [isMuted, setIsMuted] = React.useState(true);
  const [isPlaying, setIsPlaying] = React.useState(Boolean(entry.heroVideo));

  const video = entry.heroVideo;
    ...
    return (
      <section className="relative min-h-[calc(100svh-var(--nav-h))] overflow-hidden bg-black">
        <Image
          ...
          style={{
            objectPosition: entry.heroImage.objectPosition ?? "center",
          }}
        />
      </section>
    );
  ...
  return (
    <section className="relative min-h-[calc(100svh-var(--nav-h))] overflow-hidden bg-black">
      <video
        ...
        src={video.src}
        poster={video.posterSrc ?? entry.heroImage.src}
        autoPlay
        muted
        loop
        playsInline
...
function MediaButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className="flex size-11 items-center justify-center border border-white/20 bg-black/25 text-white backdrop-blur-md transition-colors duration-300 ease-luxury hover:border-white hover:bg-white hover:text-black"
    >
      {children}
    </button>
  );
